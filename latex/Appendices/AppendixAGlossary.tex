...
label set &$\set Y$ & discrete set $\set Y = \{1,\ldots, C\}$
...
class criterion loss & $\loss_\text{crit}$ & cross-entropy loss function 
statistics loss & $\loss_\varphi$ & loss/objective function for given feature-mapping $\varphi$ 
&&(defined in \ref{eqn:statistics_loss})
class-dependent & $\lossCC_\varphi$ & class-dependent formulation of loss function 
statistics loss &&(defined in \ref{eqn:class_statistics_loss})
