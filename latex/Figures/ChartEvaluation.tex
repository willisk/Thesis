...
\node (B) [node, below= of A] {\textbf{Source} \\ $\set B$};
\node (C) [node, below= 2.2cm of B, fill=blue!0] {\textbf{Validation} \\ $\set C$};

\node (Bp) [node, right= of B, fill=blue!2] {\textbf{perturbed} \\ $\widetilde {\set B} = \delta (\set B)$};
\node (Br) [node, right= of Bp, fill=blue!7] {\textbf{reconstructed} \\ $\widehat {\set B} = \rho^* (\widetilde {\set B})$};
...
\node (vNet) [function, fill=gray!5, right= of Net] {verifier \\ Neural \\ Network};
...
\node (Cp) [node, right= of C, fill=blue!0] {\textbf{perturbed} \\ $\widetilde {\set C} = \delta(\set C)$};
\node (Cr) [node, right= of Cp, fill=blue!0] {\textbf{reconstructed} \\ $\widehat {\set C} = \rho ^*(\widetilde {\set C})$};
...
\node [OliveGreen] at ($(Bp)!0.5!(Cp)$) {PSNR};
